...
- `CATEGORIES`: Dict mapping category names to coordinate keys (from `data/options.json`)
- `VILLES`: Dict mapping city names to coordinate keys (from `data/options.json`)
- `EXCEL_COLUMNS`: Mapping between internal keys and Excel column names (includes `categorie`)
